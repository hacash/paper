...
为了保证系统运行的效率，交易的数据结构设计应该尽量简单和紧凑（stupid simple but efficient work），并让人类与机器都容易理解。（金融规则的人类可读性的重要将在`设计原则`一章详细讨论）
...
可以看到，一笔交易大致分为 `actions` 、`signs` 和 `multisigns` 三个部分。

### 2. 复合签名地址

单一签名地址有秘钥丢失和被盗的风险，并且无法满足共同托管资金的需求。我们需要一种能让两个或三个私钥，以不同的权限配置来管理资金的功能，例如：
