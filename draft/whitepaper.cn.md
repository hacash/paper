...
本质上，我们无法完全解决这个问题，只能尽力改良以规避。有两种办法可以起到作用：
...
设置满足要求（即具备出块资格）的得分 x，必须 大于或等于 最近2016个区块中的 x 得分的大数平均值再减去4，也就是说，一个新加入的矿工大约要拥有最近出块矿工平均余额的 1/16 资金，才能参与挖矿。

以上两种办法可以一定程度降低算力中心化、游击挖矿和51%攻击的风险。
...
我们在`第六章、货币`已经阐述过人为控制发行量，让某些机构负责人自由裁量来保持货币价格稳定的不可行性。除此之外，我们只能依靠其市值成长到足够庞大的规模，能够消化及抵消局部的价格波动，以及借助于期货市场的套期保值，虽然这将带来额外的会计成本。

所有的货币都是不完美的，但是其中一些能够有效工作。
