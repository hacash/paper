...
当结果满足前十位为0时且后六位不包含0时，即产生了一颗钻石。根据以上结果，我们称此颗钻石的字面量（标识符）为`UKNWTH`，且字面量唯一不重复。此时将这颗钻石打包进区块并广播，所有钻石生产者停止之前的计算，而采用此新区块的哈希值重新开始计算下一颗钻石的字面值。如果同一区块时间内产生多颗钻石，则由矿工决定打包哪一颗进区块（也许是给出手续费最高的）。

钻石的总量上限约1600万颗。每挖出一颗则整体挖掘难度将相应提高，随着采掘数量的增长成指数级上升，并最终趋近于无限大。
...
我们采用先快速递增后缓慢递减的方式决定区块的货币奖励发放。第一阶段，每 10万 个区块约 0.95 年调整一次数量，奖励逐渐上升；第二阶段，每 50万 个区块约 4.75 年调整一次数量，奖励逐渐下降；第三阶段最终恒定在每区块奖励1个单位：

    1, 2, 3, 4, 5, 6, 7, 8, 8(五年), 7(五年), 6(五年), 5(五年), 4(五年), 3(五年), 2(五年), 1(五年), 1, 1, 1 ....

则前 48 年的总供应量为 2160 万单位：

    ( 1 + 2 + 3 + 4 + 5 + 6 + 7 + 8 + 8*5 + 7*5 + 6*5 + 5*5 + 4*5 + 3*5 + 2*5 + 1*5 ) * 100000  =  21600000

此后的年增发比例 `(1*100000)/0.95/21600000` 约为 0.4873%，此后逐年递减（100年后约为0.3331%，200年后约0.2530%，500年后约为0.1470%），并无限趋近于零。

#### 2) 通道利息

为促成通道链结算网络的大规模应用，我们设定以 2500 个区块约 8 天为一个结算周期，按复利奖励锁定在通道内的资金，比率为 0.01% 。假定通道资金占总量的 1/2，得出年增发率约为：

    (1+(0.0001*1/2)) ^ (365/8.33) ≈ 1.002193  ( 0.22% )

加上记账奖励，平缓时期总的年均增发量粗略估计为 0.7% ，并无限降低收敛至约 0.22% 。一个供参考的数据是1960年至2012年世界总平均GDP增长率大约为2%~4%。

#### 3) 钻石采掘

钻石的字面值由 `WTYUIAHXVMEKBSZN` 16个字符组成，后6位为字母则是合法的字面值。其总数量为：
